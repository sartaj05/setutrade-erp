...
function Wms({ mode }) {
  ...
  return <><Head eyebrow="Warehouse execution" title="Advanced WMS" text="Bin locations, release-to-pick workflows and cycle counts for tighter stock accuracy." action={createBin} actionLabel="Create demo bin"/><Notice>{notice}</Notice><div className="report-grid"><article><span>Active bins</span><strong>{data.bins.length}</strong><small>location controlled</small></article><article><span>Open picks</span><strong>{data.picks.filter(x=>x.status!=='Complete').length}</strong><small>warehouse work</small></article><article><span>Cycle counts</span><strong>{data.counts.length}</strong><small>variance checks</small></article></div><article className="panel module-panel"><Table columns={[{key:'pickNo',label:'Pick list'},{key:'warehouse',label:'Warehouse'},{key:'lines',label:'Lines'},{key:'status',label:'Status'}]} rows={data.picks}/></article><article className="panel module-panel"><Table columns={[{key:'code',label:'Bin'},{key:'zone',label:'Zone'},{key:'warehouse',label:'Warehouse'},{key:'capacity',label:'Capacity'}]} rows={data.bins}/></article></>;
}
