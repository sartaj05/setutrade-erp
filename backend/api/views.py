...
from .models import Customer, Invoice, Order, Product, Supplier, PurchaseOrder, PurchaseItem, GoodsReceipt, LedgerEntry, Warehouse, StockBalance, StockTransfer, BarcodeScanLog, WhatsAppMessage, WhatsAppOrderDraft, TaxNote, PriceList, PriceRule

PERMISSIONS = {
    'OWNER': ['dashboard','products','inventory','customers','orders','invoices','purchases','ledger','warehouses','barcode','whatsapp','tax','pricing','quotations','payments','reports','team','settings'],
    'MANAGER': ['dashboard','products','inventory','customers','orders','invoices','purchases','ledger','warehouses','barcode','whatsapp','tax','pricing','quotations','payments','reports'],
    'SALES': ['dashboard','customers','orders','invoices','ledger','whatsapp','tax','pricing','quotations','payments'],
...
def ledger(request):
    today = timezone.localdate()
    rows = LedgerEntry, Warehouse, StockBalance, StockTransfer, BarcodeScanLog, WhatsAppMessage, WhatsAppOrderDraft, TaxNote, PriceList, PriceRule.objects.select_related('customer').order_by('-entry_date', '-id')
    payload = []
    for row in rows:
...
def warehouses(request):
    ...
    transfers = StockTransfer, BarcodeScanLog, WhatsAppMessage, WhatsAppOrderDraft, TaxNote, PriceList, PriceRule.objects.select_related('from_warehouse', 'to_warehouse').prefetch_related('items').order_by('-transfer_date')[:10]
    return JsonResponse({
...
def barcode(request):
    if request.method == 'POST':
        ...
        log = BarcodeScanLog, WhatsAppMessage, WhatsAppOrderDraft, TaxNote, PriceList, PriceRule.objects.create(product=product, warehouse=warehouse, action=body.get('action', 'Lookup'), quantity=body.get('quantity', 1), scanned_by=request.api_user)
...
def whatsapp(request):
    if request.method == 'POST':
        ...
        draft = WhatsAppOrderDraft, TaxNote, PriceList, PriceRule.objects.create(draft_no=draft_no, customer=customer, raw_message=raw, parsed_items=items, estimated_total=total)
    ...
    drafts = WhatsAppOrderDraft, TaxNote, PriceList, PriceRule.objects.select_related('customer').order_by('-created_at')[:12]
...
def tax_compliance(request):
    ...
    notes = TaxNote, PriceList, PriceRule.objects.select_related('customer', 'invoice').order_by('-note_date')[:10]
    return JsonResponse({
        'tax': {
